...
def _add_label(session, host, query: str, category: str, label: str):
    req = {
        "request_id": str(uuid4()),
        "query": query,
        "operations": [
...
def test_remove_label_happy_path(datastore: HowlerDatastore, login_session):
    """Label is removed from all matching hits and reports success."""
...
# ---------------------------------------------------------------------------
# Hits without the label are skipped
# ---------------------------------------------------------------------------


def test_remove_label_not_present_skipped(datastore: HowlerDatastore, login_session):
    """Hits that do not carry the label produce a skipped outcome."""
    session, host = login_session

    # Ensure the label is absent
    resp = _execute(session, host, query="howler.id:*", category="generic", label="label-never-added")
...
def test_remove_label_invalid_category(datastore: HowlerDatastore, login_session):
...
def test_remove_label_empty_label(datastore: HowlerDatastore, login_session):
