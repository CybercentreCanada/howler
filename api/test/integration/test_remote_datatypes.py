...
def test_expiring_hash(redis_connection):
    if redis_connection:
        from howler.remote.datatypes.hash import ExpiringHash
...
def test_basic_counters(redis_connection):
    if redis_connection:
        ...
        with Counters("test-counter") as ct:
            ...
            ct.dec("t1")
            ct.dec("t2")
            assert sorted(ct.get_queues()) == ["test-counter-t1", "test-counter-t2"]
            assert ct.get_queues_sizes() == {
                "test-counter-t1": 9,
                "test-counter-t2": 39,
            }
            ct.reset_queues()
            assert ct.get_queues_sizes() == {"test-counter-t1": 0, "test-counter-t2": 0}
...
def test_unique_priority_queue(redis_connection):
    from howler.remote.datatypes.queues.priority import UniquePriorityQueue
...
def test_multi_queue(redis_connection):
    if redis_connection:
        from howler.remote.datatypes.queues.multi import MultiQueue
