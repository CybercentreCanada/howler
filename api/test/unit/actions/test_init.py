...
@patch("howler.actions.datastore")
def test_execute_admin_user_no_limit(mock_ds):
    """Test that admin users have no practical limit."""
    admin_user: User = random_model_obj(User)
    admin_user.type = ["admin", "user"]

    # Mock datastore to return a very high hit count
    mock_ds.return_value.hit.search.return_value = {"total": 5000, "items": []}
    mock_ds.return_value.hit.update_by_query.return_value = None

    result = execute("add_label", "howler.id:*", user=admin_user, category="generic", label="test")

    # Admin should not be blocked by limit
    assert all(r["title"] != "Hit limit exceeded" for r in result)
