...
def test_get_pivot_groups_scopes_search_to_visible_dossiers(mock_datastore):
    mock_datastore.return_value.dossier.search.return_value = {"aggregations": {"pivot_groups": {"buckets": []}}}
...
def test_get_pivot_groups_handles_missing_pivots(mock_datastore):
    mock_datastore.return_value.dossier.search.return_value = {
        "aggregations": {"pivot_groups": {"buckets": [{"key": {"group": "network/dns"}}]}}
    }
...
def test_get_pivot_groups_pages_before_capping(mock_datastore):
    mock_datastore.return_value.dossier.search.side_effect = [
        ...
        {
            "aggregations": {
                "pivot_groups": {"buckets": [{"key": {"group": "network/alpha"}}, {"key": {"group": "Network/Beta"}}]}
            }
        },
    ]
    ...
    assert mock_datastore.return_value.dossier.search.call_args_list[1].kwargs["aggregations"][0][1]["composite"][
        "after"
    ] == {"group": "network/zulu"}
