...
def give_privilege(
    ...
    for user_id in user_ids:
        try:
            _grant_privilege(
                str(j_request.get("privilege")), user_id.strip(), result, user, refresh, object_type.__name__.lower()
            )
        except (InvalidDataException, InvalidDataException) as e:
            errors.append((user_id, e.message))
            continue  # Chosen so we can have partial success. Failure will be clearly sent to the user

    if errors:
        error_details = "; ".join([f"{user_id}: {msg}" for user_id, msg in errors])
...
def remove_privilege(
    ...
    Data Block:
        {
            ...
            "user_id": ["user to remove permission from","Other user"],
        }
    ...
    for user_id in permission_request.user_id:
        if user_id not in (result.admins if permission_request.privilege == "admins" else result.members):
