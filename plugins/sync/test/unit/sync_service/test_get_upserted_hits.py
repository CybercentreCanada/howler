...
def test_no_interval_gets_all_hits():
    ...
    hits = sync_service.get_upserted_hits()
    assert len(hits) == 20
...
def test_interval_start_get_hits(current_time):
    ...
    hits = sync_service.get_upserted_hits(data_interval_start=start_time)
    assert len(hits) == 10
...
def test_interval_start_and_end_get_hits(current_time):
    ...
    hits = sync_service.get_upserted_hits(data_interval_start=start_time, data_interval_end=end_time)
    assert len(hits) == 5
