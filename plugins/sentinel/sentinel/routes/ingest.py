...
def ingest_xdr_incident(**kwargs) -> tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        tenant_mapping = {
            "020cd98f-1002-45b7-90ff-69fc68bdd027": "Acme Corporation"
        }
        ...
        # Step 2: Map all alerts to individual hits and create them (same as create_bundle data.get("hits"))
        ...
        for i, alert in enumerate(alerts):
            try:
                ...
                if mapped_hit:
                    ...
                    logger.debug(f"Successfully created alert hit {i}: {alert_hit_odm.howler.id}")
                else:
        ...
        try:
            # Convert bundle hit to ODM (same as create_bundle)
            ...
            # Create the bundle hit (same as create_bundle)
            ...
            for hit_id in bundle_odm.howler.hits:
                ...
                if child_hit.howler.is_bundle:
                    logger.warning(f"Child hit {child_hit.howler.id} is a bundle - skipping bundle assignment")
                    continue
            ...
            # Commit transaction and execute actions
        ...
        logger.error("Failed to process XDR incident: %s", e)
        return bad_request(err=f"Failed to process XDR incident: {str(e)}")
        
    except Exception as e:
        logger.error("Unexpected error during XDR incident ingestion: %s", e)
