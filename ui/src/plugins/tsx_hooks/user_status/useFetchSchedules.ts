import api from 'api';
import type { ScheduleBlob } from 'api/status';
...
type Options = {
   ...
   * @param data The list of schedule blobs fetched from the API.
   * @returns void
   */
  onSuccess?: (data: ScheduleBlob) => void;
  /**
...
export const useFetchSchedules = (options?: Options) => {
  const { dispatchApi } = useMyApi();

  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [data, setData] = useState<ScheduleBlob | undefined>(undefined);
  ...
  const refetch = useCallback(async () => {
    ...
    try {
      const response = await dispatchApi(api.status.getSchedules());
      setData(response);
