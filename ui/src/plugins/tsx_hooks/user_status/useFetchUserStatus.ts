...
type Options = {
   ...
   * @param data The user status fetched from the API.
   * @returns void
   */
...
export const useFetchUserStatus = (uname: string, options?: Options) => {
  ...
  const refetch = useCallback(async () => {
    ...
    try {
      const response = await dispatchApi(api.status.getUserStatus(uname));
      setData(response);
