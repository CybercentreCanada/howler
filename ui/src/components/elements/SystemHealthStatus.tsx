...
const SystemHealthStatus = () => {
  ...
  const { healthStatus, loading } = useFetchHealth({ pollingRateMS: 20000 });
  ...
  const isHealthy = () => healthStatus.every(componentHealth => componentHealth.healthy);
  const isCritical = () =>
    healthStatus
      .filter(componentHealth => componentHealth.importance === 'critical')
      .some(componentHealth => !componentHealth.healthy);
  ...
  return (
    <div>
      ...
      {loading && healthStatus.length === 0 ? loadingElement : healthyElement}
    </div>
  );
};
