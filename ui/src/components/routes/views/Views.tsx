...
const ViewsBase: FC = () => {
  ...
  const { response, request, remove, getSearchRequestData } =
    useContext<SearchResponseContextType<View>>(SearchResponseContext as any);

  const [phrase, setPhrase] = useState<string>('');
  const [offset, setOffset] = useState(parseInt(searchParams.get('offset')!) || 0);
  const [type, setType] = useState<'personal' | 'global' | null>((searchParams.get('type') as 'personal' | 'global') || null);
  ...
  const onSearch = useCallback(async () => {
    try {
      ...
      const favouritesQuery =
        favouritesOnly && (user.favourite_views?.length ?? 0) > 0 ? ` AND view_id:(${user.favourite_views!.join(' OR ')})` : '';
