...
Utilisez le contrôle de requête pour exécuter la recherche et vérifier le nombre de correspondances. L'éditeur exige un titre, un type, une requête validée, au moins une piste ou un pivot, ainsi que des détails de piste et de pivot valides avant d'activer **Sauvegarder**. Survolez **Sauvegarder** pour voir ce qui l'empêche. Une modification de la requête rend la validation précédente caduque; exécutez-la à nouveau avant de sauvegarder.
