    ...
    <Stack spacing={0} sx={{ height: '100%' }}>
      <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap" sx={{ p: 1, gap: 1 }}>
        ...
        <Autocomplete
          ...
          renderTags={(value, getTagProps) =>
            value.map((opt, index) => (
              <Chip {...getTagProps({ index })} key={opt} size="small" label={opt} color={getEscalationColor(opt)} />
            ))
          }
          renderInput={params => (
            ...
            <Stack component="li" spacing={1} key={entry.howler.id} sx={{ pb: 1 }}>
              <Stack direction="row" spacing={2} alignItems="flex-start">
                ...
                <Box sx={{ flex: 1, minWidth: 0, textDecoration: 'none', position: 'relative' }}>
                  ...
                  <Box
                    component={Link}
                    to={`/cases/${_case.case_id}/${buildPathFromID(_case, getItemId(entry.howler.id) ?? entry.howler.id)}`}
                    sx={theme => ({
                      position: 'absolute',
                      top: 0,
                      left: 0,
