...
MCP Server allowing connection between AI Agent and Howler using an On-Behalf-Of flow for authenticaion.
When the users connects for the first time, their MCP client get registered using the Dynamic Client Registration (DCR - RFC 7591) and consent is requested. More about the flows [here](README.md).   
...
The user initiates the /SearchMatchingIndicatorsInOtherSystem command, supplying the hit identifier and the target third-party system (connected via its own MCP) to perform correlation. In this example, the system used is Sentinel and the agent start by asking the output format:
...
This time the user starts interacting with another MCP server (here Microsoft Sentinel) and use the insights of these to search for matching indicators in hits in Howler.
...
This logic is simmilar to the one explained here: 🔗 https://learn.microsoft.com/en-us/graph/mcp-server/overview
